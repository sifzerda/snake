// prev snake game with collision between walls and snake with itself
// issue with second+ food consumed triggering game over
...
const SnakeGame = () => {
  ...
  const lastSegmentAddedRef = useRef(null);
 ...
 useEffect(() => {
  ...
  const checkCollision = () => {
    ...
    if (Matter.Query.collides(head, [food]).length > 0) {
      ...
      lastSegmentAddedRef.current = newSegment; // Track the new segment
    }

    // Check self-collision
    for (let i = 1; i < snake.length; i++) {
      const segment = snake[i];
      if (Matter.Query.collides(head, [segment]).length > 0 && segment !== lastSegmentAddedRef.current) {
        setGameOver(true);
        //playCollisionSound();
        break;
      }
